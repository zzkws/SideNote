...
**图也能问。** 按 `Alt+S` 框住一张图、一个表格、或者排版成图片的公式，它会告诉你：坐标轴和图例各是什么、这张图想让你看出什么结论、证据在图上哪一处。看不清的地方它会直说，不猜。

V4.1 Flash 原生支持视觉理解，所以这条走的是同一个模型、同一个 Key。一张 800px 宽的插图约 330 token，成本可以忽略。截图在 service worker 里裁好并缩到长边 1000px 再发。

**查过的都存着。** 点工具栏图标 →「历史对话」，能看到查过的每个词和当时的追问，可以按词、按文章标题、按答案内容搜，也能一键回到原文。只存在本机 `chrome.storage.local`，最多留最近 200 条。

**PDF 也能读。** Chrome 自带的 PDF 查看器内部是 PDFium 插件不是网页，扩展拿不到选区 —— 所以内置了一个 PDF 阅读器页面。设置页点「打开 PDF 阅读器」，把 PDF 拖进去，划词体验跟网页上一样。

PDF 里没有段落这个概念，文字层只是一堆按坐标散落的碎片。阅读器会按坐标把行、栏、段重新拼回去：检测栏间空隙分左右栏、按右边界判断段末、合并行末连字符、滤掉页码。实测在一篇双栏 CVPR 论文上，摘要正确还原成完整一段。

**长句也能选。** 上限放到 400 字符 / 60 个英文词，够划两三句话。选中整句时第一块讲这段在说什么、把长句主干拆出来，英文释意和文化拆解落到其中最关键的那个表达上。

**不触发的只有三种**：纯空白、纯标点、明显误选整段（超过 400 字符或 60 个英文词）。单个字母、缩写、希腊字母、百分比都会触发 —— 读论文时它们恰恰最需要解释。
